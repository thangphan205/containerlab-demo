...
```mermaid
graph TD
    subgraph transit ["Transit Router Chain"]
        direction LR
    ...
    subgraph lan1 ["LAN 1 (10.0.1.0/24)"]
        host-1["host-1<br>(unconfigured)"]
    end

    subgraph lan2 ["LAN 2 (10.0.2.0/24)"]
        host-2["host-2<br>(unconfigured)"]
    end

    r1 -- "eth1 <-> eth1<br>(10.0.1.0/24)" --- host-1
    r4 -- "eth2 <-> eth1<br>(10.0.2.0/24)" --- host-2
```
